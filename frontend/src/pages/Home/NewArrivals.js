...
export default class NewArrivals {
    ...
    setProducts(products) {
        // Filter new arrival watches
        this.products = products.filter(p => parseInt(p.is_new) === 1);
        this.renderGrid();
    }

    renderGrid() {
        if (!this.element) return;
        const grid = this.element.querySelector('#newArrivalsGrid');
        if (!grid) return;
        ...
        section.innerHTML = `
            <div class="max-w-[1400px] mx-auto px-6">
                <div class="flex justify-between items-end mb-12 border-b border-accent/50 dark:border-accent/40 pb-5">
                    <h2 class="font-serif text-3xl md:text-4xl text-primary dark:text-white font-normal">Sản Phẩm Mới</h2>
                    <a href="#/products?category=all&sort=newest" class="text-xs font-bold tracking-widest uppercase pb-1 border-b border-primary dark:border-white text-primary dark:text-white hover:text-accent dark:hover:text-accent hover:border-accent dark:hover:border-accent transition-all duration-300 font-sans">XEM TẤT CẢ</a>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8" id="newArrivalsGrid">
                    <!-- Loading placeholder -->
                    <div class="col-span-full text-center py-16 text-gray-400">
                        <i class="fa-solid fa-spinner fa-spin mr-2"></i> Đang tải sản phẩm mới...
                    </div>
                </div>
            </div>
        `;
