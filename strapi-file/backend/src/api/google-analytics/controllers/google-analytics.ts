...
const PROPERTY_ID_PATH = path.join(process.cwd(), 'google-property-id.txt');
...
async function queryGA4Report(accessToken: string, propertyId: string, requestBody: any) {
...
export default {
  ...
  async saveConfig(ctx) {
    try {
      ...
      fs.writeFileSync(CREDENTIALS_PATH, JSON.stringify(parsedSA, null, 2), 'utf-8');
      fs.writeFileSync(PROPERTY_ID_PATH, propertyId.toString().trim(), 'utf-8');
  ...
  async getReport(ctx) {
    const hasCredentials = fs.existsSync(CREDENTIALS_PATH) && fs.existsSync(PROPERTY_ID_PATH);
    ...
    try {
      ...
      const propertyId = fs.readFileSync(PROPERTY_ID_PATH, 'utf-8').trim();
...
function parseTrafficReport(report: any) {
  const rows = report.rows || [];
  let totalSessions7Days = 0;
  let totalDuration = 0;
  let activeUsersToday = 0;

  const chartData = rows.map((row: any, idx: number) => {
...
function parsePagesReport(report: any) {
  const rows = report.rows || [];
...
function generateSimulatedData(localRevenue: number, localOrdersCount: number) {
  return {
    metrics: {
